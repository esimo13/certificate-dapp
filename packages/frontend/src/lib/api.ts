...
export const apiClient = {
  ...
  // Transfer module APIs
  getUniversities: async () => {
    const response = await fetch(`${API_BASE_URL}/transfer/universities`)
    return response.json()
  },

  getCourses: async (uniId: string) => {
    const response = await fetch(`${API_BASE_URL}/transfer/courses?uniId=${encodeURIComponent(uniId)}`)
    return response.json()
  },

  calculateTransfer: async (payload: {
    sourceUniversityId: string
    targetUniversityId: string
    sourceCourseIds: string[]
    eligibilityProof?: string
  }) => {
    const response = await fetch(`${API_BASE_URL}/transfer/calculate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  compareSyllabus: async (syllabusA: string, syllabusB: string) => {
    const response = await fetch(`${API_BASE_URL}/transfer/syllabus-compare`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  compareSyllabusFromPdf: async (sourceSyllabusPdf: File, targetSyllabusPdf: File) => {
    const formData = new FormData()
    formData.append('sourceSyllabusPdf', sourceSyllabusPdf)
    formData.append('targetSyllabusPdf', targetSyllabusPdf)

    const response = await fetch(`${API_BASE_URL}/transfer/syllabus-compare-pdf`, {
      method: 'POST',
      body: formData,
    })
